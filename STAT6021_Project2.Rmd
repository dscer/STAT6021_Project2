...
for (i in col_names){
    plot<-ggplot(df, aes(x=df[[i]], color=df$waterfront))+
      geom_density()+
      labs(x="waterfront", y=i, title=paste0())
    print(plot)
}
```
...
plot(waterfront ~ price.log, data=df, col="steelblue")
...
```{r}
reduced<-glm(waterfront~price+bathrooms+view+grade+yr_renovated, family="binomial", data=train)
summary(reduced)
```
